        ...
        try {
            ...
            for (int t = 1; t <= cases; t++) {
                ...
                outer: for (int i = 0; i < startTimes.size()-1; i++) {
                    int overlaps = 0;
                    LocalTime startA = startTimes.get(i);
                    LocalTime stopA = endTimes.get(i);
                    for (int j = i+1; j < startTimes.size(); j++) {
                        LocalTime startB = startTimes.get(j);
                        LocalTime stopB = endTimes.get(j);
                        if (startA.isBefore(stopB) && stopA.isAfter(startB) || startB.isBefore(stopA) && stopB.isAfter(startA)) {
                            overlaps++;
                            if(resultArr[i] !=null && resultArr[j]==null) {
                                if(resultArr[i].equals("C")) {
                                    resultArr[j]="J";
                                } else {
                                    resultArr[j]="C";
                                }
